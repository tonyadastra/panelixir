...
$(document).ready(function () {
    ...
    // var speed = 1e-2, start = Date.now(), sphere = { type: "Sphere" };
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                Promise.all(files.map(url => d3.json(url))).then(function (values) {
                    ...
                    // context.clearRect(0, 0, map_svgW, map_svgH);
                                        ...
                                        (function transition() {
                                            d3.select(".clicked").transition()
                                                ...
                                                .tween("rotate", function () {
                                                    let r;
                                                    if (map_continent == 'Europe') {
                                                        r = d3.interpolate(projection.rotate(), [-25.187509595656408, -42.75594328392682]);
                                                    } else if (map_continent == 'North America') {
                                                        r = d3.interpolate(projection.rotate(), [96.3959652111984, -60.4768931916812]);
                                                    } else if (map_continent == 'Antarctica') {
                                                        r = d3.interpolate(projection.rotate(), [-69.53089507427761, 89.30702018723926]);
                                                    } else if (map_continent == 'Oceania') {
                                                        r = d3.interpolate(projection.rotate(), [-138.56893652314406, 18.31504815749142]);
                                                    } else if (map_continent == 'Asia') {
                                                        r = d3.interpolate(projection.rotate(), [-96.51319759040699, -20.94479984182297]);
                                                    } else if (map_continent == 'South America') {
                                                        r = d3.interpolate(projection.rotate(), [60.17789122485408, 20.310911906837475]);
                                                    } else if (map_continent == 'Africa') {
                                                        r = d3.interpolate(projection.rotate(), [-29.91403682471926, 3.377695631236064]);
                                                    }
