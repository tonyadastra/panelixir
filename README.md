...
7. Zip all necessary files to upload to AWS Lambda. Remember to include new files here if necessary
    + `zip -g function.zip AutoUpdate.py`
8. Send updates to AWS Lambda
    + `aws lambda update-function-code --function-name auto_update_google --zip-file fileb://function.zip`
...
+ `aws lambda invoke --function-name auto_update_google return.json --log-type Tail --query 'LogResult' --output text |  base64 -d`
